...
/**
 * An entity's origin key — the signing identity its facts hang off. The ONE
 * precedence order, used for the origin address in originFacts, the grouping
 * in stampOriginClaims, and nowhere else twice.
 */
export function originKey(entity) {
 ...
 * the entity set — in ONE aggregation, not one query per entity — and stamps
 * `_originClaims` so originFacts can report it as a derived fact. On failure we
 * leave it unset; the facts then omit `claims` rather than emit a faked zero.
 * Callers pass the Mongo db handle — this module holds no connection.
 */
export async function stampOriginClaims(db, entities) {
  const origins = [...new Set((entities || []).map(e => originKey(e)).filter(Boolean))];
  if (origins.length === 0) return;

  // Group by the same precedence originKey applies, expressed for Mongo.
  ...
  try {
    ...
    console.warn('[present] origin claims enrichment failed:', err.message);
    return;
  }

  for (const e of entities) {
    const key = originKey(e);
 ...
 * event Scan observed. `claims` (history depth) is the per-origin count
 * stamped by stampOriginClaims when the caller ran it — never a faked zero.
 */
export function originFacts(entity) {
  const m = entity.metadata || {};
  const sig = m.verification || m.signature || {};
  const facts = {
    address: originKey(entity),
