 ...
 * gracefully on exit.
 *
 * Config: { "mcp.epistery.com": "/home/.../mcp-registry", ... }
...
export default class Harness {
  ...
  async _spawn(hostname, cwd, port) {
    // Kill any stale process left on this port from a previous run
    try {
      const pid = execSync(`lsof -ti tcp:${port}`, { encoding: 'utf8' }).trim();
      if (pid) {
        console.log(`[harness] killing stale process on port ${port} (pid ${pid})`);
  ...
  /**
   * POST to a specific child by hostname.
   * Returns { hostname, data } or null on error/timeout.
   */
  async post(hostname, urlPath, body, timeout = 5000) {
    const child = this.children[hostname];
    if (!child || !child.healthy) return null;
    return this._post(child.port, urlPath, hostname, body, timeout);
  }

  _post(port, urlPath, hostname, body, timeout) {
    return new Promise((resolve) => {
      ...
      const req = http.request(
        ...
        (res) => {
          ...
          res.on('end', () => {
            try {
              resolve({ hostname, data: JSON.parse(data) });
            } catch {
              resolve(null);
            }
          });
