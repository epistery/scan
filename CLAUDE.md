...
**The live server (`/opt/epistery-scan` on the scan host) is always git-synced. The user deploys manually from git.**
...
- The `secrets.json` and similar files in `/opt/epistery-scan/` exist on the server but are not in the repo. Leave them alone unless the user asks.
...
`ssh ubuntu@epistery.io` still resolves to the scan server until DNS repoint; after that use `ubuntu@epistery.com`.
